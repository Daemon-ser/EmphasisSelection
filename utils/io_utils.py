...
def convert_to_slide_format(words_id,words,mean_pred_label):
    slide_words_id, slide_words, slide_mean_pred_label=[],[],[]
    slides_words_id, slides_words, slides_mean_pred_label=[],[],[]
    pre_s='0'
    for i in range(len(words_id)):
        for j in range(len(words_id[i])):
            _, s, l, w = words_id[i][j].split('-')
            if s == pre_s:
                slide_words_id.append(words_id[i][j])
                slide_words.append(words[i][j])
                slide_mean_pred_label.append(mean_pred_label[i][j])
            else:
                slides_words_id.append(slide_words_id)
                slides_words.append(slide_words)
                slides_mean_pred_label.append(slide_mean_pred_label)
                slide_words_id, slide_words, slide_mean_pred_label = [], [], []
                slide_words_id.append(words_id[i][j])
                slide_words.append(words[i][j])
                slide_mean_pred_label.append(mean_pred_label[i][j])
    ...
    slides_words.append(slide_words)
    slides_mean_pred_label.append(slide_mean_pred_label)
    return   slides_words_id, slides_words, slides_mean_pred_label
